...
const consentimientoTextos = {
 ...
 En el marco del desarrollo del presente proyecto de investigación, se le invita a participar de manera voluntaria en este estudio, cuyo propósito es analizar la relación entre los factores emoc[...]

 Yo, identificado(a) con el número de cédula que aparece al pie de mi firma, actuando en nombre propio, manifiesto que he sido informado(a) de manera clara, suficiente y comprensible, y mis pregu[...]

 1. Información del estudio
 He sido informado(a) de manera clara y suficiente sobre el objetivo del estudio, el cual busca analizar la relación entre los hábitos parafuncionales orales y los factores emocionales, así como[...]
 ...
 3. Riesgos
 Se me ha informado que esta investigación es de riesgo mínimo, ya que no implica procedimientos invasivos ni intervenciones que afecten mi integridad física o psicológica. En concordancia con [...]
 ...
 Se garantiza que la información suministrada será tratada con estricta confidencialidad y utilizada únicamente con fines académicos e investigativos. Mi identidad no será revelada en publicac[...]
 ...
 En el marco del desarrollo del presente proyecto de investigación, se solicita su autorización para la toma de registros fotográficos intraorales, los cuales serán utilizados exclusivamente co[...]

 Yo, identificado(a) con el número de cédula que aparece al pie de mi firma, actuando en nombre propio, manifiesto que he sido informado(a) de manera clara, suficiente y comprensible, y mis pregu[...]

 1. Finalidad de los registros fotográficos
 He sido informado(a) de manera clara de que las imágenes serán utilizadas exclusivamente con fines académicos, temáticos e investigativos en el desarrollo del trabajo de grado.
 ...
 Se garantiza que las imágenes serán tratadas con estricta confidencialidad, evitando cualquier información que permita mi identificación. En caso de que se requiera incluir elementos que pueda[...]

 5. Uso de las imágenes
 Autorizo que los registros fotográficos puedan ser utilizados en análisis clínico, presentaciones académicas o temáticas y publicaciones derivadas de la investigación.
 ...
 Declaro que he recibido información suficiente sobre el propósito, alcance y uso de los registros fotográficos, y que he tenido la oportunidad de realizar preguntas, las cuales han sido respond[...]
...
const s = {
  ...
  container: { maxWidth: '700px', width: '100%', backgroundColor: '#111218', borderRadius: '12px', padding: '25px', border: '1px solid rgba(255, 255, 255, 0.06)', margin: '10px auto', boxShadow: '[...]' },
  ...
  btn: { backgroundColor: '#1b1d26', color: '#fff', border: '1px solid rgba(255, 255, 255, 0.1)', borderRadius: '8px', padding: '12px 24px', fontSize: '16px', fontWeight: '500', cursor: 'pointer',[...] },
  btnSecondary: { backgroundColor: '#111218', color: '#9ca3af', border: '1px solid rgba(255, 255, 255, 0.06)', borderRadius: '8px', padding: '12px 24px', fontSize: '16px', cursor: 'pointer', width[...] },
  btnSuccess: { backgroundColor: 'rgba(16, 185, 129, 0.12)', color: '#34d399', border: '1px solid rgba(16, 185, 129, 0.3)', borderRadius: '8px', padding: '12px 24px', fontSize: '16px', fontWeight:[...] },
  input: { width: '100%', padding: '12px 15px', margin: '5px 0 15px 0', border: '1px solid rgba(255, 255, 255, 0.08)', borderRadius: '6px', fontSize: '15px', backgroundColor: '#15161e', color: '#f[...] },
  label: { display: 'block', marginBottom: '5px', color: '#9ca3af', fontWeight: '500', fontSize: '14px' },
  formGroup: { marginBottom: '10px' },
  error: { color: '#f87171', fontSize: '14px', margin: '10px 0', padding: '12px', backgroundColor: 'rgba(239, 68, 68, 0.12)', borderRadius: '6px', textAlign: 'center', border: '1px solid rgba(239,[...]' },
  ...
  consentBox: { backgroundColor: '#0d0d12', color: '#e5e7eb', padding: '20px', borderRadius: '8px', marginBottom: '20px', maxHeight: '350px', overflowY: 'auto', fontSize: '13px', lineHeight: '1.6'[...]
}
      ...
      <div style={s.container}>
        ...
        {pantalla === 'cedulaCons1' && (
          <div>
            ...
            <div style={s.formGroup}><label style={s.label}>Nombres completos:</label><input style={s.input} value={cons1.nombre} onChange={e => setCons1({ ...cons1, nombre: e.target.value })} pl[...]
            <div style={s.formGroup}><label style={s.label}>Apellidos completos:</label><input style={s.input} value={cons1.apellidos} onChange={e => setCons1({ ...cons1, apellidos: e.target.valu[...]
            <div style={s.formGroup}><label style={s.label}>Número de cédula:</label><input style={s.input} type="number" value={cons1.cedula} onChange={e => setCons1({ ...cons1, cedula: e.targ[...]
        ...
        {pantalla === 'cedulaCons2' && (
          <div>
            ...
            <div style={s.formGroup}><label style={s.label}>Número de cédula:</label><input style={s.input} type="number" value={cons2Cedula} onChange={e => setCons2Cedula(e.target.value)} plac[...]
            {error && <div style={s.error}>{error}</div>}
        ...
        {pantalla === 'consentimiento' && (
          <div>
            ...
            <div style={s.infoPaciente}>
              <p style={{ color: '#fff', fontSize: '16px' }}>Paciente: <strong>{tipoConsentimiento === 1 ? `${cons1.nombre} ${cons1.apellidos}` : `${paciente?.nombre} ${paciente?.apellidos}`}</st[...]
