...
/**
 * Renumera los IDs de consentimientos para que coincidan con el id del paciente:
 * C1 (tipo 1) queda con el id del paciente y C2 (tipo 2) con id del paciente + 1000
 */
export async function renumerarConsentimientos() {
  const { data: restantes, error: errSel } = await supabase
  ...
  // Primero mover todos los IDs a un valor temporal para evitar conflictos
  for (let i = 0; i < rows.length; i++) {
    const { error: e } = await supabase
      .from('consentimientos')
      .update({ id: rows[i].id + OFFSET })
      .eq('id', rows[i].id)
    if (e) throw e
  }

  // Luego reasignar según el id del paciente: C1 = id paciente, C2 = id paciente + 1000
  for (let i = 0; i < rows.length; i++) {
    const nuevoId = rows[i].tipo === 2 ? rows[i].paciente_id + 1000 : rows[i].paciente_id
    const { error: e } = await supabase
      .from('consentimientos')
      .update({ id: nuevoId })
      .eq('id', rows[i].id + OFFSET)
    if (e) throw e
  }
}

/**
 * Elimina un consentimiento por su ID y renumera los IDs restantes
 * para que el siguiente pase a ocupar el lugar del eliminado
 */
export async function deleteConsentimiento(id) {
  const { error } = await supabase.from('consentimientos').delete().eq('id', id)
  if (error) throw error
  await renumerarConsentimientos()
}

/**
 * Elimina todos los consentimientos (C1 y C2) de un paciente por su cédula y renumera
 */
