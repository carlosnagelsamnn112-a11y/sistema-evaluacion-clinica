...
export default function useFlujoPaciente() {
  ...
  // Bloqueo de reintentos concurrentes (evita duplicados por doble clic o clics repetidos)
  ...
  const validarCedulaFlujo = useCallback(async (tipo) => {
    ...
    try {
      ...
      if (tipo === 'historia') {
        const tieneHistoria = (data.historia || []).some(h => h.fecha_nacimiento && h.fecha_nacimiento !== '')
        if (tieneHistoria) {
      ...
      if (tipo === 'encuesta') {
        ...
        const tieneEncuesta = (data.encuesta || []).some(r => r.p1 != null)
        if (tieneEncuesta) {
