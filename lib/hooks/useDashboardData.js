...
import {
  ...
  deleteConsentimientosDePaciente,
...
export default function useDashboardData() {
  ...
  const handleEliminarConsentimientos = useCallback(async (cedula, pacNombre, tipos) => {
    if (!confirm(`Advertencia: Se eliminarán los consentimientos de ${pacNombre} (${tipos}).\n\n¿Está seguro de que desea eliminarlos?`)) return
    try {
      await deleteConsentimientosDePaciente(cedula)
      alert('Consentimientos eliminados con éxito.')
      await cargarDatos()
    } catch (e) {
      console.error('Error al eliminar consentimientos:', e)
      alert('Error al eliminar consentimientos: ' + e.message)
    }
  }, [cargarDatos])

  const handleEditarPaciente = useCallback(async (paciente, datos) => {
    try {
      await updatePacienteDatos(paciente.id, paciente.cedula, datos)
      alert('Datos del paciente actualizados con éxito.')
      await cargarDatos()
    } catch (e) {
      console.error('Error al actualizar paciente:', e)
      alert('Error al actualizar paciente: ' + e.message)
    }
  }, [cargarDatos])
  ...
  return {
    ...
    eliminarConsentimientos: handleEliminarConsentimientos,
